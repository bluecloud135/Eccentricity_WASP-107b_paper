...
class ModelFunc(object):
    def __init__(self, kind, fixed_params=None):
        self.kind = kind
    ...
    def get_model(self):
        if self.kind == 'Linear':
            ...
            return self.pdotModel
        elif self.kind == 'Precession':
            print('Using Precession model')
            return self.apsidalPrecessionModel
        elif self.kind == 'Sine':
            print('Using Sine model')
            return self.sineModel
        else:
            raise ValueError("Not a valid kind of model")
        
    def linearModel(self, params,N):
        if self.fixed_params is not None:
            params = list(self.fixed_params) + list(params)
        t0, period = params
        t_tras = [np.nan]*(N+1)
        t_tras[0] = t0
    ...
    def apsidalPrecessionModel(self,params,N):
        if self.fixed_params is not None:
            params = list(self.fixed_params) + list(params)
